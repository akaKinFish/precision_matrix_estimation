function [Omega_est, Sigma_src_est, outs] = solver_jspace_3d_opt_bayesopt(Svv_cell, L, GraphLaplacian, cfg)
% SOLVER_JSPACE_3D_OPT_BAYESOPT - Scheme-1 (eta-search) using Bayesian Optimization
%
% Revisions:
%   - [STABILITY] Added 'stabilize_corr_spd_' to fix ill-conditioned Sjj_tilde.
%   - [OPTIMIZER] Fixed Warm Start: Now actively fills 'init_pts' to meet N_INIT.
%   - [OPTIMIZER] Dynamic 'NumSeedPoints' to satisfy bayesopt requirements.
%   - [FIX] Changed default 'bo_num_seed_points' to 2 (MATLAB requires >0).
%   - [OPTIMIZER] Replaced 'surrogateopt' with 'bayesopt' for smarter exploration.
...
% [STABILITY] Stabilize Sjj_tilde (floor + normalize)
min_eig_S = get_cfg(cfg,'whiten_min_eig', 1e-4);
for f = 1:F
    S_cpu = gather(Sjj_tilde0{f});
    S_cpu = stabilize_corr_spd_(S_cpu, min_eig_S);
    Sjj_tilde0{f} = S_cpu;
end
...
% ============================================================
% 8) Surrogate optimization on eta (BAYESOPT BRANCH)
% ============================================================

% ---- 8.1 Read global bounds (log10) ----
...
% ---- 8.2 Shrink bounds (same as surrogateopt) ----
eta0 = [get_cfg(cfg,'eta1_init',0.1), get_cfg(cfg,'eta2_init',0.2), get_cfg(cfg,'eta3_init',0.1)];
if isfield(cfg,'eta0') && numel(cfg.eta0)==3, eta0 = cfg.eta0(:).'; end
eta0 = max(eta0, eta_global_lb);
eta0 = min(eta0, eta_global_ub);
x0_log = log10(eta0);

DO_SHRINK  = get_cfg(cfg, 'eta_shrink_enable', true);
SH_DEC     = get_cfg(cfg, 'eta_shrink_decades', 0.7);
SH_MODE    = lower(get_cfg(cfg, 'eta_shrink_mode', 'prev'));

eta_prev = [];
if isfield(cfg,'eta_prev') && ~isempty(cfg.eta_prev), eta_prev = cfg.eta_prev(:).'; end
eta_bank = [];
if isfield(cfg,'eta_bank') && ~isempty(cfg.eta_bank) && size(cfg.eta_bank,2)==3, eta_bank = cfg.eta_bank; end
...
if DO_SHRINK
    ...
    if any(lb >= ub), lb = eta_global_lb; ub = eta_global_ub; end
end
...
if VERBOSE
    fprintf('[J-SPACE-3D] bayesopt bounds (log10): lb=[%.2f %.2f %.2f], ub=[%.2f %.2f %.2f]\n', ...
        lb_log(1),lb_log(2),lb_log(3), ub_log(1),ub_log(2),ub_log(3));
end

% ---- 8.3 Prepare Initial Points (Warm Start) ----
...
% Limit size for bayesopt initial set
max_init_safe = min(15, size(init_pts,1));
init_pts = init_pts(1:max_init_safe, :);

% [FIX] Actively fill with random points if fewer than N_INIT
if SEED ~= 0, rng(SEED,'twister'); end
n_need = max(0, N_INIT - size(init_pts,1));
if n_need > 0
    rnd = rand(n_need, 3);
    extra = lb_log + rnd .* (ub_log - lb_log);
    init_pts = [init_pts; extra];
end

if VERBOSE
    fprintf('[J-SPACE-3D] bayesopt warm-start seeds: %d\n', size(init_pts,1));
end

% ---- 8.4 Define Objective Wrapper ----
...
obj_fun_bo = @(T) obj_fun([T.x1, T.x2, T.x3]);

% ---- 8.5 Run bayesopt ----
if VERBOSE
    fprintf('[J-SPACE-3D] Running bayesopt on eta (log-space)...\n');
end

vars = [
    optimizableVariable('x1', [lb_log(1), ub_log(1)], 'Type', 'real')
    optimizableVariable('x2', [lb_log(2), ub_log(2)], 'Type', 'real')
    optimizableVariable('x3', [lb_log(3), ub_log(3)], 'Type', 'real')
];

% Convert init points to table for InitialX
init_tbl = array2table(init_pts, 'VariableNames', {'x1','x2','x3'});

BO_ACQ   = get_cfg(cfg, 'bo_acquisition', 'expected-improvement-plus');
BO_EXPL  = get_cfg(cfg, 'bo_exploration_ratio', 0.5);
BO_DET   = get_cfg(cfg, 'bo_is_deterministic', true);

% [FIX] Dynamic NumSeedPoints (must be >= 1, and >0 if no InitialX)
% We set it to fill the gap if user asked for more seeds, or default to 2
seed_total = get_cfg(cfg, 'bo_num_seed_points', 6);
BO_SEEDS   = max(1, seed_total - height(init_tbl)); 

BO_SEED  = get_cfg(cfg, 'bo_rng_seed', 0);
if BO_SEED ~= 0, rng(BO_SEED, 'twister'); end

tic_opt = tic;
results = bayesopt( ...
    obj_fun_bo, vars, ...
    'MaxObjectiveEvaluations', MAX_OPT_EVALS, ...
    'IsObjectiveDeterministic', BO_DET, ...
    'UseParallel', USE_PARALLEL, ...
    'Verbose', double(VERBOSE), ...
    'PlotFcn', [], ...
    'AcquisitionFunctionName', BO_ACQ, ...
    'ExplorationRatio', BO_EXPL, ...
    'NumSeedPoints', BO_SEEDS, ...
    'InitialX', init_tbl);
time_opt = toc(tic_opt);

% Extract Best
x_best_log = [results.XAtMinObjective.x1, results.XAtMinObjective.x2, results.XAtMinObjective.x3];
fval       = results.MinObjective;
eta_best   = 10.^x_best_log;

outs = struct();
outs.opt_results.fval = fval;
outs.opt_results.exitflag = 1; 
outs.opt_results.output = results; 
outs.opt_results.trials = struct();

% Convert trace to linear space for compatibility
Xtrace_log = [results.XTrace.x1, results.XTrace.x2, results.XTrace.x3];
outs.opt_results.trials.X    = 10.^Xtrace_log; 
outs.opt_results.trials.Fval = results.ObjectiveTrace(:);
...
for em_iter = 1:MAX_EM_ITER
    ...
    % [STABILITY] Stabilize Sjj_tilde inside EM
    for f = 1:F
        S_cpu = gather(Sjj_tilde{f});
        S_cpu = stabilize_corr_spd_(S_cpu, min_eig_S);
        Sjj_tilde{f} = S_cpu;
        m5_in_base.whitened_covariances{f} = S_cpu;
    end
...
% Forces SPD floor and re-normalizes diagonal to 1 (correlation-like)
    S = (S + S')/2;
    [S, ~] = utils_math.project_spd(S, min_eig);   % eigen-floor
    d = real(diag(S)); d = max(d, 1e-12);
    S = S ./ sqrt(d*d');                           % renormalize diag ~ 1
