...
% ---------------- Term 4: single-frequency spatial smoothing (lambda3) ---------------
spatial_term = 0;
if isfield(params,'lambda3') && params.lambda3 > 0 && isfield(params,'spatial_graph_matrix') ...
        && ~isempty(params.spatial_graph_matrix)
    try
        sp_out = module5_spatial_smoothing_singlefreq(Gamma_cells, struct( ...
            ...
            'spatial_graph_is_laplacian', getfield(params,'spatial_graph_is_laplacian',true), ...
            'spatial_weight_mode',       getfield(params,'spatial_weight_mode','node'), ...
            'return_gradient', false, ...
            'validate_inputs', true, ...
            'enforce_hermitian_grad', false ...
        ));
        spatial_term = sp_out.term;
    catch ME
        warning('module5_objective:spatial_eval_failed','%s',ME.message);
        spatial_term = 0;
    end
end

% ---------------- Combine ----------------
objective_value = loglik_term + smoothing_term + l1_term + spatial_term;

if ~isfinite(objective_value)
    warning('module5_objective:non_finite_objective', ...
        'Objective not finite (loglik=%.2e, smooth=%.2e, l1=%.2e, spatial=%.2e)', ...
        loglik_term, smoothing_term, l1_term, spatial_term);
end
